 ...
 */
function initializeOpportunitiesPage(): void {
    ...
    if (signUpForm) {
        signUpForm.addEventListener("submit", function (event) {
            ...
            if (userName && userEmail && userRole) {
                const popupBody = document.querySelector("#signUpPopup .modal-body") as HTMLElement;
                if (popupBody) {
 ...
 */
function displayOpportunities(filteredOpportunities: Opportunity[]): void {
    const opportunitiesList = document.getElementById("opportunities-list") as HTMLElement;
    if (!opportunitiesList) {
 ...
 */
function setPopupData(title: string): void {
 ...
 * @param address
 */
function viewLocation(address: string): void {
    ...
    fetch('data/OpenCageGeoCodingAPIKEY')
        ...
        .then((data) => {
            if (data.results && data.results[0]?.geometry) {
                ...
                const modalBody = document.getElementById('locationModalBody') as HTMLElement;
                if (modalBody) {
                    modalBody.innerHTML = `<p>Address: ${address}</p><p>Coordinates: ${coordinates}</p>`;
                }
                const locationModal = new bootstrap.Modal(document.getElementById('locationModal') as HTMLElement);
                locationModal.show();
            } else {
